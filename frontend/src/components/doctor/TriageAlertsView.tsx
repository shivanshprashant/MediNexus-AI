    ...
    <div className="flex flex-col w-full px-4 pt-2 pb-36 max-w-lg mx-auto">
      ...
      {activeSos && (
        <div className="bg-surface-container-lowest rounded-xl shadow-sm p-4 flex flex-col gap-3 relative overflow-hidden border border-outline-variant/30 mb-6">
          <div className={`absolute top-0 left-0 right-0 h-1.5 ${activeSos.status === 'accepted' ? 'bg-primary' : 'bg-error'}`}></div>
          <div className="flex items-start justify-between gap-3 pt-1">
            <div className="flex gap-3 items-center">
              <div className="w-11 h-11 rounded-xl bg-error-container text-on-error-container flex items-center justify-center font-bold text-base">
                {activeSos.patientName ? activeSos.patientName.substring(0, 2).toUpperCase() : 'ER'}
              </div>
              <div>
                <div className="flex items-center gap-2">
                  <span className="font-headline-md text-[16px] font-bold text-on-surface">{activeSos.patientName}</span>
                  <span className="bg-surface-container-high font-data-mono text-[11px] px-1.5 py-0.5 rounded">
                    {activeSos.age || '29'} {activeSos.gender?.charAt(0) || 'F'}
                  </span>
                </div>
                <span className="font-data-mono text-[11px] text-on-surface-variant">Mode: {activeSos.mode === 'drive-in' ? 'Drive-In' : 'Ambulance'}</span>
              </div>
            </div>
            <span className={`px-2.5 py-0.5 rounded-full font-label-caps text-[10px] uppercase font-bold ${activeSos.status === 'accepted' ? 'bg-primary text-white' : 'bg-error-container text-on-error-container'}`}>
              {activeSos.status === 'accepted' ? 'Assigned' : '1 Urgent'}
            </span>
          </div>
          ...
          <div className="bg-surface-container-low rounded-lg p-3">
            ...
            <p className="font-body-md text-[13px] text-on-surface mt-1">{activeSos.complaint || 'Severe emergency symptoms on arrival.'}</p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2 bg-surface-container rounded-lg p-2.5 text-[12px]">
            <div>
              ...
              <strong className="text-on-surface">{activeSos.hospitalName || 'City Care Hospital'}</strong>
              <div className="font-data-mono text-[11px] text-on-surface-variant">{activeSos.bedNo || 'ER'}</div>
            </div>
            <div>
              <span className="font-label-caps text-[10px] text-on-surface-variant uppercase block">Status</span>
              <strong className="text-on-surface">{activeSos.status === 'en-route' ? 'En-route' : 'Arrived'}</strong>
              <div className="font-data-mono text-[11px] text-primary font-bold">Priority Triage</div>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-2 pt-1">
            <button onClick={onOpenEHR} className="bg-surface-container-high py-2.5 px-3 rounded-lg font-label-caps text-xs uppercase font-bold flex items-center justify-center gap-1 cursor-pointer hover:bg-surface-container-highest transition-colors">
              <span className="material-symbols-outlined text-[17px]">folder_shared</span> View EHR
            </button>
            <button onClick={onOpenEmergencyIntake} className="bg-surface-container-high py-2.5 px-3 rounded-lg font-label-caps text-xs uppercase font-bold flex items-center justify-center gap-1 cursor-pointer hover:bg-surface-container-highest transition-colors">
              <span className="material-symbols-outlined text-[17px]">clinical_notes</span> Triage Intake
            </button>
          </div>

          <button
            onClick={onOpenEmergencyIntake}
            className={`w-full py-3 rounded-lg font-label-caps text-xs uppercase font-bold flex items-center justify-center gap-2 shadow-sm cursor-pointer ${
              activeSos.status === 'accepted' ? 'bg-surface-container-high text-on-surface-variant' : 'bg-primary text-white hover:bg-primary-container'
            }`}
          >
            <span className="material-symbols-outlined text-[18px]">assignment_turned_in</span>
            <span>{activeSos.status === 'accepted' ? 'Acknowledged & Active' : 'Acknowledge Case (Take In / Divert)'}</span>
          </button>
        </div>
      )}
