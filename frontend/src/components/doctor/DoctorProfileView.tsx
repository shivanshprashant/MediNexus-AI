...
export const DoctorProfileView: React.FC<DoctorProfileViewProps> = ({
  ...
  const mapProfile = (dp?: any) => ({
    name: dp?.name || '',
    title: dp?.title || dp?.specialization || '',
    hospital: dp?.facility || '',
    specialization: dp?.specialization || '',
    qualification: dp?.qualification || '',
    license: dp?.license || '',
    abhaId: dp?.doctor_code || dp?.id || '',
    room: dp?.room || '',
    hours: dp?.shift || '',
    phone: dp?.contactPhone || dp?.contact_phone || '',
    photo: dp?.photo || '',
    availability: dp?.availability || 'ON DUTY',
  });

  const [docProfile, setDocProfile] = useState(mapProfile(doctorProfile));
  const [editForm, setEditForm] = useState({ ...docProfile });

  useEffect(() => {
    if (doctorProfile) {
      const mapped = mapProfile(doctorProfile);
      setDocProfile(mapped);
      setAvailableOpd(mapped.availability === 'ON DUTY');
    }
  }, [doctorProfile]);
  ...
  const handlePhotoSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      onShowToast('Please select a JPG, PNG, or WEBP image');
      return;
    }

    if (file.size > 5 * 1024 * 1024) {
      onShowToast('Image file size must be less than 5 MB');
      return;
    }
  ...
  const handleSaveDocProfile = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsSaving(true);
    try {
      if (selectedPhotoFile) {
        await uploadDoctorPhotoApi(selectedPhotoFile);
      }
      await updateDoctorProfileApi({
        name: editForm.name,
        title: editForm.title,
        facility: editForm.hospital,
        specialization: editForm.specialization,
        qualification: editForm.qualification,
        license: editForm.license,
        room: editForm.room,
        shift: editForm.hours,
        contactPhone: editForm.phone,
      });

      const freshProfile = await fetchDoctorProfileApi();
      if (freshProfile) {
        const mapped = mapProfile(freshProfile);
        setDocProfile(mapped);
        if (onProfileUpdated) onProfileUpdated(freshProfile);
      } else {
        setDocProfile({ ...editForm, photo: photoPreviewUrl || docProfile.photo });
      }
  ...
  const displayPhoto = photoPreviewUrl || (
    docProfile.photo
      ? (docProfile.photo.startsWith('http') ? docProfile.photo : `http://localhost:8000${docProfile.photo}`)
      : defaultPhoto
  );
    ...
    <div className="flex flex-col w-full px-4 pt-2 pb-36 max-w-lg mx-auto">
      <div className="pt-2 pb-3 flex items-center justify-between">
        <div>
          ...
          <p className="font-label-caps text-xs text-on-surface-variant uppercase">Clinical Station • ID #{docProfile.abhaId}</p>
        </div>
        <button
          type="button"
          onClick={() => {
      ...
      <div className="bg-surface-container-lowest rounded-xl p-4 shadow-sm border border-outline-variant/30 flex flex-col gap-3 mb-4">
        ...
        <div className="grid grid-cols-3 gap-2 pt-1 text-center">
          ...
          <div className="bg-surface-container-low rounded-lg p-2">
            ...
            <span className="text-[9px] text-secondary">{docProfile.specialization}</span>
          </div>
          <div className="bg-surface-container-low rounded-lg p-2">
      ...
      <div className="bg-surface-container-lowest rounded-xl p-4 shadow-sm border border-outline-variant/30 flex flex-col gap-3 mb-4">
        ...
        <div className="flex items-center justify-between p-2.5 rounded-lg bg-surface-container-low">
          ...
          <button
            type="button"
            onClick={async () => {
              const nextState = !availableOpd;
              setAvailableOpd(nextState);
              const doctorId = doctorProfile?.id || doctorProfile?.doctor_code || docProfile?.abhaId;
              if (!doctorId) return;
              try {
                await updateDoctorStatusApi(doctorId, nextState ? 'ON DUTY' : 'OFF DUTY');
              } catch (err) {
                console.warn('Update doctor status API fallback:', err);
              }
              onShowToast(nextState ? 'Available for OPD' : 'Duty Paused');
            }}
            className={`w-11 h-6 rounded-full transition-colors cursor-pointer relative flex items-center px-1 ${availableOpd ? 'bg-primary' : 'bg-gray-300'}`}
          >
            <span className={`inline-block w-4 h-4 bg-white rounded-full transition-transform ${availableOpd ? 'translate-x-5' : 'translate-x-0'}`} />
          </button>
        </div>
            ...
            <form onSubmit={handleSaveDocProfile} className="py-4 space-y-3 text-xs">
              ...
              <div className="flex items-center gap-3 p-3 bg-surface-container-low rounded-xl">
                <img
                  ...
                  src={photoPreviewUrl || (docProfile.photo ? (docProfile.photo.startsWith('http') ? docProfile.photo : `http://localhost:8000${docProfile.photo}`) : defaultPhoto)}
                />
                <div className="flex flex-col gap-1">
                  <span className="font-bold text-xs text-on-surface">Profile Photo</span>
                  <span className="text-[10px] text-on-surface-variant">JPG, PNG, or WEBP (Max 5 MB)</span>
                  <button
                    type="button"
                    onClick={() => fileInputRef.current?.click()}
                    className="px-3 py-1 bg-primary text-white rounded-lg font-bold text-[11px] hover:bg-primary-container cursor-pointer self-start transition-colors"
                  >
                    Change Photo
                  </button>
                  <input
              ...
              <div className="pt-2 flex gap-2">
                <button
                  type="submit"
                  disabled={isSaving}
                  className="flex-1 py-3 bg-primary text-white font-bold text-xs uppercase rounded-xl shadow-sm cursor-pointer hover:bg-primary-container disabled:opacity-50"
                >
